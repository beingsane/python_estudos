...
# -*- coding: iso-8859-1 -*-
#-------------------------------------------------------------------------------
# -- Sistema de Cadastro de Alunos Utilizando Classes --
#-------------------------------------------------------------------------------
...
#----------------------------------
def altera(aluno):
    print "\n---- Alterando os dados de %s ----"%aluno.Name
    while True:
...
def excluir(): 
    ...
    while True:
        try:
            ...
            print "- Aluno inexistente..."
    print "\n%d- %s, %s"%(ra,alunos[ra].Name,alunos[ra].Curso)
...
def gravarFile(filename):
    ...
    try:
        with file(filename,'w') as arq:
            for ra in alunos.iterkeys():
                arq.write("%d,%s,%s;"%(ra,alunos[ra].Name,alunos[ra].Curso))
                arq.flush()
    except IOError:
    ...
    try:
        for line in alunosA:
            ...
            if ra in alunos.iterkeys():
                op=raw_input("Conflito de Ra: %s já existe com esse ra, deseja substituir o existente pelo novo? (s) ou (n): "%(alunos[ra].Name))
                if op!='s': continue
            alunos[ra] = a.Aluno(ra,nome,curso) 
